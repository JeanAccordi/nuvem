...
$sql = mysqli_query($conexao,"select * from usuarios_loja_padrao where email_usu='".anti_sql_injection($_POST['email_usu'])."' and senha_usu='".anti_sql_injection($_POST['senha_usu'])."' limit 1") or die("Erro ao validar");
...
if($linhas == '')
	{
		?>
         <div class="msg2 padding20">Usuário não encontrado ou senha inválida, tente novamente.</div>
                ...
                setTimeout("window.location='acesso_loja.html';",2000);
                </script>
        <?PHP
	}
else
	{
		...
		$_SESSION['nome_loja_sessao'] = $dados['nome_loja'];
				header("Location: conteudo-loja.php");
			}
	}
?>
            </div>
